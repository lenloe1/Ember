...
#ifndef GATEWAY_APP
  ignoreNextEzspError = EZSP_SPI_ERR_WAIT_SECTION_TIMEOUT;
#else
  ignoreNextEzspError = EZSP_ASH_HOST_FATAL_ERROR;
#endif
  ezspStat = ezspLaunchStandaloneBootloader(mode);
  if (ignoreNextEzspError != EZSP_SUCCESS) {
