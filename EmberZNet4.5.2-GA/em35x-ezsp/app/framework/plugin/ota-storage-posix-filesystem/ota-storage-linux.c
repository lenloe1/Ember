...
#include "ota-storage-linux.h"
  ...
  || defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_SERVER)
...
// Forward Declarations
  ...
  #define strnlen(s, maxlen) myStrnlen((s), (maxlen))
#else
  #define myStrnlen(s, maxlen) strnlen((s), (maxlen))
#endif
...
EmberAfOtaStorageStatus emberAfOtaStorageClearTempDataCallback(void)
{
  ...
  // This is actually 1 byte too long, because strlen() includes
  // the '\0' for each string.  So subtract 1.
  ...
  if (MAX_FILEPATH_LENGTH < snprintf(tempStorageFilepath, 
                                     MAX_FILEPATH_LENGTH,
                                     "%s%s",
                                     storageDevice,
                                     tempStorageFile)) {
  ...
  // Windows requires the 'b' (binary) as part of the mode so that line endings
  // are not truncated.  POSIX ignores this.
  FILE* fileHandle = fopen(tempStorageFilepath,
                                    ...
                                    boolean printImageInfo)
{
  ...
  int length = 1 + myStrnlen(filename, OTA_MAX_FILENAME_LENGTH);
