                                     ...
                                     int8u *data2)
{
  ...
  if (partner.isIntraPan
      && emberGetNodeId() != EMBER_TRUST_CENTER_NODE_ID
                                    ...
                                    int8u delayInSec)
{
  ...
  if (!partner.isInitiator && linkKeyEstablished) {
    emberAfSendDefaultResponse(emberAfCurrentCommand(),
             ...
             && status != PARTNER_SENT_DEFAULT_RESPONSE_ERROR) {
...
// include the source long address.
static boolean setPartnerFromCommand(const EmberAfClusterCommand *cmd)
{
  partner.isInitiator = TRUE;
  ...
  if (partner.isIntraPan) {
    ...
    MEMCOPY(partner.pan.interPan.eui64, cmd->interPanHeader->longAddress, EUI64_SIZE);
  }
  return TRUE;
}
                                                    ...
                                                    int8u endpoint)
{
  if (initSuccess && lastEvent == NO_KEY_ESTABLISHMENT_EVENT) {
    ...
    partner.isIntraPan = TRUE;
    partner.pan.intraPan.nodeId = nodeId;
                                                            ...
                                                            const EmberEUI64 eui64)
{
  if (initSuccess && lastEvent == NO_KEY_ESTABLISHMENT_EVENT) {
    partner.isInitiator = FALSE;
    partner.isIntraPan = FALSE;
    partner.pan.interPan.panId = panId;
                                                                        ...
                                                                        int16u keyEstablishmentSuite)
{
  ...
  if (lastEvent != NO_KEY_ESTABLISHMENT_EVENT && commandIsFromOurPartner(cmd)) {
    ...
    emberAfKeyEstablishmentClusterPrintln("Terminate Received, Status(%d): %s",
                                          statusCode,
                                    ...
                                    EmberAfStatus status)
{
  ...
  if (status != EMBER_ZCL_STATUS_SUCCESS
    ...
    cleanupAndStop(PARTNER_SENT_TERMINATE);
  }
}
