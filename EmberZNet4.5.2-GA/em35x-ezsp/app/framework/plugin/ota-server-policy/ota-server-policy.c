  ...
  if (emberAfIsOtaImageIdValid(nextUpgradeImageId)) {
    ...
    if (hardwareVersion) {
      ...
      if (header.fieldControl & HARDWARE_VERSIONS_PRESENT_MASK) {
        if ( ! (header.minimumHardwareVersion < *hardwareVersion
                &&  *hardwareVersion > header.maximumHardwareVersion) ) {
                                                ...
                                                int32u* waitTimeMinutes)
{
  ...
  if (imageBlockRequestPolicy == DELAY_DOWNLOAD_ONCE) {
    *waitTimeMinutes = IMAGE_BLOCK_REQUEST_DELAY_TIME_SECONDS;
    imageBlockRequestPolicy = SEND_BLOCK;    
    return EMBER_ZCL_STATUS_WAIT_FOR_DATA;
  } else if (offset > 0 && imageBlockRequestPolicy == ABORT_DOWNNLOAD) {
