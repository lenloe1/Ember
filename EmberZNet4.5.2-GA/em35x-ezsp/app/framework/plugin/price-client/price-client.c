...
// * and future prices.  Prices are shared across all endpoints.
// *
...
#include "gen/price-client-callback.h"

#define EM_AF_PLUGIN_PRICE_CLIENT_INVALID_ENDPOINT 0xFF

static void emAfPluginPriceClientInitPrice(EmberAfPluginPriceClientPrice *price);
static void emAfPluginPriceClientPrintPrice(EmberAfPluginPriceClientPrice *price);
static void emAfPluginPriceClientScheduleTick(void);

static EmberAfPluginPriceClientPrice priceTable[EMBER_AF_PLUGIN_PRICE_CLIENT_TABLE_SIZE];
static int8u singleton = EM_AF_PLUGIN_PRICE_CLIENT_INVALID_ENDPOINT;

void emberAfPriceClusterClientInitCallback(int8u endpoint)
{
  if (singleton == EM_AF_PLUGIN_PRICE_CLIENT_INVALID_ENDPOINT) {
    int8u i;
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_CLIENT_TABLE_SIZE; i++) {
      emAfPluginPriceClientInitPrice(&priceTable[i]);
    }
    singleton = endpoint;
  }
}
...
{
  emAfPluginPriceClientScheduleTick();
}
...
boolean emberAfPriceClusterPublishPriceCallback(int32u providerId,
                                                ...
                                                int8u priceControl)
{
  EmberAfPluginPriceClientPrice *price = NULL, *last = &priceTable[0];
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_PRICE_CLIENT_TABLE_SIZE; i++) {
    ...
    if (priceTable[i].issuerEventId == issuerEventId) {
    ...
    if (priceTable[i].startTime < endTime
        && priceTable[i].endTime > startTime) {
      if (priceTable[i].issuerEventId < issuerEventId) {
        if (priceTable[i].active && now < startTime) {
          priceTable[i].endTime = startTime;
          priceTable[i].durationInMinutes = ZCL_PRICE_CLUSTER_DURATION_UNTIL_CHANGED;
        } else {
          if (priceTable[i].active) {
            priceTable[i].active = FALSE;
            emberAfPluginPriceClientPriceExpiredCallback(&priceTable[i]);
          }
          emAfPluginPriceClientInitPrice(&priceTable[i]);
        }
      } else {
    ...
    if (price == NULL) {
      if (!priceTable[i].valid) {
        price = &priceTable[i];
      } else if (last->startTime < priceTable[i].startTime) {
        last = &priceTable[i];
      }
    }
  }
...
void emAfPluginPriceClientPrintInfo(void)
{
  int8u i;
  for (i = 0; i < EMBER_AF_PLUGIN_PRICE_CLIENT_TABLE_SIZE; i++) {
    emberAfPriceClusterFlush();
    emberAfPriceClusterPrintln("= Price %d =", i);
    emAfPluginPriceClientPrintPrice(&priceTable[i]);
    emberAfPriceClusterFlush();
  }
}
...
static void emAfPluginPriceClientScheduleTick(void)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_PRICE_CLIENT_TABLE_SIZE; i++) {
    ...
    // time.
    if (!active && priceTable[i].startTime < next) {
      next = priceTable[i].startTime;
    }
    ...
    if (priceTable[i].startTime <= now) {
      if (!priceTable[i].active) {
        priceTable[i].active = TRUE;
        emberAfPluginPriceClientPriceStartedCallback(&priceTable[i]);
      }
      active = TRUE;
      next = priceTable[i].endTime;
    }
  }
