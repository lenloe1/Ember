...
int8u PGM emSerialTxQueueWraps[EM_NUM_SERIAL_PORTS] = 
  { FOR_EACH_PORT( , EMBER_SERIAL,_TX_QUEUE_SIZE-1 ) }; 
int8u PGM emSerialRxQueueWraps[EM_NUM_SERIAL_PORTS] = 
  { FOR_EACH_PORT( , EMBER_SERIAL,_RX_QUEUE_SIZE-1 ) }; 
#else
...
static int8u decimalStringWrite(int32s value, 
                                boolean signedValue, 
                                int8u* buffer)
{
  int8u length = 0;
  ...
  for (; *string; string++) {
    ...
    if (next != '%')
      ...
      case 'd': {       // signed 2-byte
        ...
        localBufferPointer += decimalStringWrite(value,
                                                 (*string != 'u'),  // signed value?
                                                 localBufferPointer);
        break;
      }
      case 'x':
